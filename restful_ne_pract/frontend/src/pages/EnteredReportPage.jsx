...
export default function EnteredReportPage() {
  ...
  const fetchReport = async (p = page) => {
    ...
    setError('');
    try {
      const res = await api.get('/api/reports/entered', { params: { ...dates, page: p, limit: 10 } });
  ...
  const handleDownloadCsv = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get('/api/reports/entered/download', {
        params: dates,
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', `entered_report_${dates.startDate}_to_${dates.endDate}.csv`);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white">Entered Cars Report</h1>
        <p className="text-slate-400 text-sm mt-1">Audit all cars that entered the parking slots within select dates</p>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="card border-l-4 border-l-indigo-500 relative overflow-hidden">
            ...
            <p className="text-xs uppercase font-bold text-slate-500 tracking-wider">Total Cars Entered</p>
            <p className="text-3xl font-extrabold text-white mt-2">{summary.totalRecords} Vehicles</p>
          </div>
        </div>
      )}
